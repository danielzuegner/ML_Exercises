function [ action ] = policySelectAction( state, values, gridSize, actions, gamma )
    ...
    for target_state=1:(gridSize*gridSize)
        reward = -1;
        if state == 1 || state == 16 % terminal state
            reward = 0;
        end
        for a=actions %iterate over actions
            ...
            action_values(a) = action_values(a) + state_probabilities(target_state) * (reward + gamma*values(target_state));
        end
    end
